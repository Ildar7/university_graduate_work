import { getGender as getGenderModel, getGenderById as getGenderByIdModel } from '../models/Gender.js';

export const getGenders = async (req, res) => {
    try {
        const { page = 1, limit = 50, sort = 'id_gender', order = 'asc' } = req.query;
        const validSortFields = ['id_gender'];
        ...
        const { data, total } = await getGenderModel({
            page: pageNum,
            limit: limitNum,
            sort,
...
export const getGenderById = async (req, res) => {
    try {
        ...
        const item = await getGenderByIdModel(parsedId);
        if (!item) {
        return res.status(404).json({ message: `Запись с ID ${parsedId} не найдена` });
    }
        res.json(item);
    } catch (error) {
        console.error(`Ошибка при получении пола по ID:`, error);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
};
