import { getCitizenship as getCitizenshipModel, getCitizenshipById as getCitizenshipByIdModel } from '../models/Citizenship.js';

export const getCitizenships = async (req, res) => {
    try {
        const { page = 1, limit = 50, sort = 'id_citizenship', order = 'asc' } = req.query;
        const validSortFields = ['id_citizenship'];
        ...
        const { data, total } = await getCitizenshipModel({
            page: pageNum,
            limit: limitNum,
            sort,
        ...
        console.error(`Ошибка при получении гражданств:`, error);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
};

export const getCitizenshipById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const item = await getCitizenshipByIdModel(parsedId);
        if (!item) {
            return res.status(404).json({ message: `Запись с ID ${parsedId} не найдена` });
        }
        res.json(item);
    } catch (error) {
        console.error(`Ошибка при получении гражданства по ID:`, error);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
};
