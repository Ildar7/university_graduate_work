import { classNames } from 'shared/lib/helpers/classNames/classNames';
import {
    CButton, CForm, CModal, CModalBody, CModalFooter, CModalHeader, CModalTitle, CToaster,
} from '@coreui/react';
import { Skeleton } from 'shared/ui/Skeleton/Skeleton';
import CIcon from '@coreui/icons-react';
import { cilSave, cilTrash, cilWarning } from '@coreui/icons';
import {
    Text, TextSize, TextTheme, TextWeight,
} from 'shared/ui/Text/Text';
import React, {
    ReactElement, useEffect, useRef, useState,
} from 'react';
import { ArrivalSourcesType } from 'entities/ArrivalSources';
import { useAppDispatch } from 'shared/lib/hooks/useAppDispatch/useAppDispatch';
import { useSelector } from 'react-redux';
import { Toast } from 'shared/ui/Toast/Toast';
import { CFormInputWithMask } from 'shared/ui/InputMask/InputMask';
import { editArrivalSourceActions } from 'features/ArrivalSources/EditArrivalSource';
import {
    fetchArrivalSourceDetail,
    getArrivalSourceDetailData,
    getArrivalSourceDetailError,
    getArrivalSourceDetailIsLoading,
} from 'entities/ArrivalSourceDetail';
import {
    editArrivalSource,
} from 'features/ArrivalSources/EditArrivalSource/model/services/editArrivalSource/editArrivalSource';
import { Button, ButtonTheme } from 'shared/ui/Button/Button';
import {
    getEditArrivalSourceIsLoading,
} from '../model/selectors/getEditArrivalSourceIsLoading/getEditArrivalSourceIsLoading';
import { getEditArrivalSourceError } from '../model/selectors/getEditArrivalSourceError/getEditArrivalSourceError';
import cls from './EditArrivalSource.module.scss';
import {
    getEditArrivalSourceNewFieldsData,
} from '../model/selectors/getEditArrivalSourceNewFieldsData/getEditArrivalSourceNewFieldsData';

interface EditArrivalSourceProps {
    className?: string;
    visible: boolean;
    setVisible: (value: boolean) => void;
    arrivalSourceId: string;
    onDeleteArrivalSource: (arrivalSource: ArrivalSourcesType) => void;
}
export const EditArrivalSource = (props: EditArrivalSourceProps) => {
    const {
        className,
        visible,
        setVisible,
        arrivalSourceId,
        onDeleteArrivalSource,
    } = props;
    const dispatch = useAppDispatch();
    const [toast, addToast] = useState<ReactElement>();
    const toaster = useRef<HTMLDivElement | null>(null);
    const [validated, setValidated] = useState<boolean>(false);
    const [formWithErrors, setFormWithErrors] = useState<boolean>(false);

    const arrivalSourceDetailData = useSelector(getArrivalSourceDetailData);
    const isLoadingArrivalSourceDetail = useSelector(getArrivalSourceDetailIsLoading);
    const errorArrivalSourceDetail = useSelector(getArrivalSourceDetailError);

    const isLoadingEditArrivalSource = useSelector(getEditArrivalSourceIsLoading);
    const editArrivalSourceNewFieldsData = useSelector(getEditArrivalSourceNewFieldsData);
    const errorsEditArrivalSource = useSelector(getEditArrivalSourceError);

    useEffect(() => {
        if (arrivalSourceId) {
            dispatch(fetchArrivalSourceDetail(arrivalSourceId));
        }
    }, [dispatch, arrivalSourceId]);

    const onCloseModal = () => {
        setVisible(false);
        setValidated(false);
        setFormWithErrors(false);
    };

    const onCancelHandler = () => {
        onCloseModal();
        dispatch(editArrivalSourceActions.clearNewFields());
    };

    const onDeleteHandler = (arrivalSource: ArrivalSourcesType) => {
        onDeleteArrivalSource(arrivalSource);
        onCloseModal();
    };

    const onChangeInput = (event: React.ChangeEvent<HTMLInputElement>) => {
        dispatch(editArrivalSourceActions.setTitle(event.target.value));
    };

    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        const form = event.currentTarget;
        setFormWithErrors(!form.checkValidity());
        if (!form.checkValidity()) {
            event.stopPropagation();
        }
        setValidated(true);

        if (form.checkValidity()) {
            if (arrivalSourceDetailData?.comesfrom === editArrivalSourceNewFieldsData) {
                addToast(Toast.error('Для сохранения вы должны внести изменения!'));
            } else {
                const result = await dispatch(editArrivalSource(arrivalSourceId));

                if (result.meta.requestStatus === 'fulfilled') {
                    addToast(Toast.success('Информация о месте рождения успешно обновлена'));
                    onCancelHandler();
                }

                if (result.meta.requestStatus === 'rejected') {
                    setFormWithErrors(true);
                }
            }
        }
    };

    let content;

    if (isLoadingEditArrivalSource || isLoadingArrivalSourceDetail) {
        content = (
            <Skeleton width="100%" height={80} />
        );
    } else if (errorArrivalSourceDetail) {
        content = (
            <Text
                theme={TextTheme.ERROR}
                size={TextSize.M}
                weight={TextWeight.SEMIBOLD}
            >
                Произошла ошибка при загрузке данных, попробуйте перезагрузить страницу
            </Text>
        );
    } else {
        content = (
            <div className={cls.tab}>
                <div className={cls.tabBlock}>
                    <div className={cls.settings}>
                        <div className={cls.newAddField}>
                            <h6 className={cls.newAddFieldTitle}>Наименование</h6>
                            <CFormInputWithMask
                                type="text"
                                placeholder="другой области"
                                // @ts-ignore
                                feedbackValid={errorsEditArrivalSource ? '' : 'Здорово!'}
                                invalid={!!errorsEditArrivalSource}
                                feedbackInvalid="Введите корректные данные"
                                value={editArrivalSourceNewFieldsData || ''}
                                onChange={onChangeInput}
                            />
                        </div>
                    </div>
                </div>
            </div>
        );
    }

    return (
        <>
            <CModal
                className={classNames(cls.EditArrivalSource, {}, [className])}
                visible={visible}
                onClose={onCloseModal}
                size="lg"
                alignment="center"
            >
                <CForm
                    className={classNames(cls.form, {}, ['needs-validation'])}
                    noValidate
                    validated={validated}
                    onSubmit={handleSubmit}
                >
                    <CModalHeader>
                        {
                            isLoadingEditArrivalSource || isLoadingArrivalSourceDetail
                                ? <Skeleton width={200} height={30} />
                                : (
                                    <CModalTitle>
                                        Изменение места рождения №
                                        {arrivalSourceDetailData?.id_comesfrom}
                                    </CModalTitle>
                                )
                        }
                    </CModalHeader>
                    <CModalBody>
                        {content}
                    </CModalBody>
                    <CModalFooter
                        className={cls.footer}
                    >
                        <div className={cls.message}>
                            {(formWithErrors) && (
                                <>
                                    <CIcon icon={cilWarning} customClassName={cls.messageIcon} />
                                    <Text
                                        size={TextSize.M}
                                        weight={TextWeight.SEMIBOLD}
                                    >
                                        Форма заполнена некорректно
                                    </Text>
                                </>
                            )}
                        </div>
                        <div className={cls.footerBtns}>
                            <Button
                                theme={ButtonTheme.OUTLINE}
                                className={cls.footerBtn}
                                onClick={onCancelHandler}
                            >
                                <Text
                                    size={TextSize.XS}
                                    weight={TextWeight.SEMIBOLD}
                                >
                                    Отмена
                                </Text>
                            </Button>
                            <Button
                                theme={ButtonTheme.ERROR}
                                className={classNames(cls.footerBtn, {}, [cls.redBtn])}
                                onClick={() => { onDeleteHandler(arrivalSourceDetailData!); }}
                                disabled={isLoadingArrivalSourceDetail
                                    || isLoadingEditArrivalSource
                                    || !!errorArrivalSourceDetail}
                            >
                                <Text
                                    size={TextSize.XS}
                                    weight={TextWeight.SEMIBOLD}
                                >
                                    Удалить
                                </Text>
                                <CIcon icon={cilTrash} className={cls.btnIcon} />
                            </Button>
                            <Button
                                type="submit"
                                className={classNames(cls.footerBtn, {}, [cls.greenBtn])}
                                disabled={isLoadingArrivalSourceDetail
                                    || isLoadingEditArrivalSource
                                    || !!errorArrivalSourceDetail}
                            >
                                <Text
                                    size={TextSize.XS}
                                    weight={TextWeight.SEMIBOLD}
                                >
                                    Сохранить
                                </Text>
                                <CIcon icon={cilSave} className={cls.btnIcon} />
                            </Button>
                        </div>
                    </CModalFooter>
                </CForm>
            </CModal>

            <CToaster
                ref={toaster}
                push={toast}
                placement="top-end"
            />
        </>
    );
};
