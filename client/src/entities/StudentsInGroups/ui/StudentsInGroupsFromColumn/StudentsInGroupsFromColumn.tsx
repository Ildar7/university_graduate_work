...
import cls from './StudentsInGroupsFromColumn.module.scss';
...
export const StudentsInGroupsFromColumn = memo((props: Props) => {
    const {
        className,
        value,
        onChangeValue,
    ...
    const fetchFilteredGroups = async () => {
        ...
        try {
            ...
            dispatch(studentsInGroupsActions.setDataFrom((res.data.data as StudentsType[]).map((student) => (
                {
                    ...student,
                    fio: `${student.user.last_name} ${student.user.first_name} ${student.user.patronymic || ''}`,
                    checked: false,
                ...
                <div className={cls.studentsList}>
                    {searchedData && searchedData.length
                        ? searchedData.map((student) => (
                            ...
                            >
                                <Checkbox
                                    id={`students-in-groups-from-${student.students_id}`}
